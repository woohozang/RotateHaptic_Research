...
/// </summary>
public class RotateHapticCombinedV3 : MonoBehaviour
{
    [Header("기준 프레임")]
    ...
    private void DistributeHaptics(float baseAmp, float freq, float yawAbs, float dt)
    {
        ...
        else
        {
            ...
            // 저항 손(Anchor)에게 줄 가변 저항값 계산
            // 주도 손이 빠를수록(힘을 쓸수록) 저항 손의 진동도 같이 커지게 설정 (토크 모사)
            float resistanceWeight = Mathf.Lerp(weakAmp, strongAmp * 0.6f, handSpeedFactor);

            targetL = leftDominant ? strongAmp * baseAmp : resistanceWeight * baseAmp;
            targetR = leftDominant ? resistanceWeight * baseAmp : strongAmp * baseAmp;
        }

        // 4. 각 손의 최종 진폭에 최소 임계값 적용 (이 부분이 중요합니다)
        if (_phase != Phase.Idle)
        {
            // 주도 손은 strong하게, 버티는 손도 최소한 느낄 수 있는 수준(minAmpWhileActive)을 보장
            targetL = Mathf.Max(targetL, minAmpWhileActive * (lS > rS ? 1.0f : 0.8f));
            targetR = Mathf.Max(targetR, minAmpWhileActive * (rS > lS ? 1.0f : 0.8f));
        }
