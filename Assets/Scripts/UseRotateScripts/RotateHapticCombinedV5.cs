...
/// </summary>
public class RotateHapticCombinedV5 : MonoBehaviour
{
    [Header("기준 프레임")]
    public Transform motionFrame;

    [Header("회전 각속도 매핑 설정")]
    [Tooltip("컨트롤러 회전 속도에 따른 햅틱 추가 가중치")]
    public float angularSpeedGain = 0.15f;
    [Tooltip("최대 각속도 기준 (라디안/초, 보통 10~15 정도가 최대 손목 회전)")]
    public float maxAngularSpeed = 12.0f;
    [Tooltip("회전 진동의 최소 베이스 라인 (느리게 돌아도 느껴지게)")]
    public float minRotationAmp = 0.1f;
    ...
    private void DistributeHaptics(float baseAmp, float freq, float yawAbs, float dt)
    {
        ...
        // [핵심 추가] 컨트롤러의 각속도(Angular Velocity) 가져오기
        Vector3 lAngV = OVRInput.GetLocalControllerAngularVelocity(OVRInput.Controller.LTouch);
        Vector3 rAngV = OVRInput.GetLocalControllerAngularVelocity(OVRInput.Controller.RTouch);

        float lS = (lv.magnitude < handSpeedDeadzone) ? 0f : lv.magnitude;
        float rS = (rv.magnitude < handSpeedDeadzone) ? 0f : rv.magnitude;

        // 각속도 크기 계산
        float lAngS = lAngV.magnitude;
        float rAngS = rAngV.magnitude;
        float maxAngSpeed = Mathf.Max(lAngS, rAngS);

        // 2. 각속도를 햅틱 강도에 매핑 (0~1 범위로 정규화)
        // 사용자가 손목을 빨리 돌릴수록 rotationFactor가 커짐
        float rotationFactor = Mathf.Clamp01(maxAngSpeed / maxAngularSpeed);

        // 연속적인 햅틱을 위해 베이스 진폭에 회전 가중치 추가
        // 카트가 멈춰있어도 손목을 돌리면 미세한 진동이 느껴지도록 minRotationAmp와 배합
        float combinedRotationAmp = (rotationFactor * angularSpeedGain) + (yawAbs > yawThresholdDeg ? minRotationAmp : 0);

        // 기존 선속도 기반 baseAmp와 회전 기반 진폭을 합산
        float finalBaseAmp = baseAmp + combinedRotationAmp;

        // 3. 양손 주도권 밸런싱 (이전 로직 유지)
        float targetL, targetR;
        float speedRatio = (Mathf.Max(lS, rS) > 0.01f) ? Mathf.Min(lS, rS) / Mathf.Max(lS, rS) : 1f;

        if (speedRatio >= bothHandsSimilarity)
        {
            float midAmp = (strongAmp + weakAmp) * 0.5f;
            targetL = targetR = midAmp * finalBaseAmp * bothHandsBoost;
        }
        else
        {
            bool leftDominant = (lS + lAngS > rS + rAngS); // 선속도와 각속도를 모두 고려하여 주도권 판별
            targetL = leftDominant ? strongAmp * finalBaseAmp : weakAmp * finalBaseAmp;
            targetR = leftDominant ? weakAmp * finalBaseAmp : strongAmp * finalBaseAmp;
        }

        // 4. 부드러운 적용
        if (_phase == Phase.Idle && yawAbs <= yawThresholdDeg && maxAngSpeed < 0.5f)
        {
