...
/// </summary>
public class RotateHapticCombinedV2_2 : MonoBehaviour
{
    [Header("안정화 설정")]
    public float switchingThreshold = 0.05f; // 주도권 전환을 위한 최소 속도 차이

    private bool _lastLeftDominant = true; // 이전 프레임의 주도권 기억
    ...
    private void DistributeHaptics(float baseAmp, float freq, float yawAbs, float dt)
    {
        ...
        // 1. 양손 협동 모드 (유사도가 높을 때)
        if (speedRatio >= bothHandsSimilarity)
        {
            float midAmp = (strongAmp + weakAmp) * 0.5f;
            targetL = targetR = midAmp * baseAmp * bothHandsBoost;
        }
        else
        {
            // 2. [핵심] 주도권 유지 로직 (히스테리시스)
            // 두 손의 속도 차이가 일정 수준(switchingThreshold) 이상 날 때만 주도권을 바꿈
            if (Mathf.Abs(lS - rS) > switchingThreshold)
            {
                _lastLeftDominant = (lS > rS);
            }

            // 리버스 로직 적용: 주도하는 손에 weakAmp 할당
            targetL = _lastLeftDominant ? weakAmp * baseAmp : strongAmp * baseAmp;
            targetR = _lastLeftDominant ? strongAmp * baseAmp : weakAmp * baseAmp;
        }

        // 3. 더 부드러운 전이 (smooth 값을 높이거나 dt를 활용)
        // 끊기는 느낌이 강하다면 인스펙터에서 smooth 값을 8~10 정도로 낮춰보세요.
