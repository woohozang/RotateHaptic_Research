...
public class RotateHapticCombinedV2 : MonoBehaviour
{
    ...
    private void DistributeHaptics(float baseAmp, float freq, float yawAbs, float dt)
    {
        ...
        // 두 손의 속도가 유사(bothHandsSimilarity 이상)할수록 햅틱 차이를 줄임
        if (speedRatio >= bothHandsSimilarity)
        {
            // 양손이 거의 동시에 움직임 -> 동일한 햅틱값 (평균값 사용)
        ...
        {
            // 주도권이 명확할 때만 비대칭 햅틱 적용
            bool leftDominant = (lS > rS);
            targetL = leftDominant ? strongAmp * baseAmp : weakAmp * baseAmp;
            targetR = leftDominant ? weakAmp * baseAmp : strongAmp * baseAmp;
        }
